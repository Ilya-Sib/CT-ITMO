...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route, HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            method.invoke(page, getMethodParameters(method.getParameterTypes(), request, view));
    ...
    private Object[] getMethodParameters(Class<?>[] methodParameterClasses,
                                         HttpServletRequest request, Map<String, Object> view) {
        List<Object> result = new ArrayList<>();
        for (Class<?> parameterClass : methodParameterClasses) {
            if (HttpServletRequest.class.equals(parameterClass)) {
                result.add(request);
            } else if (Map.class.equals(parameterClass)) {
                result.add(view);
            }
        }
