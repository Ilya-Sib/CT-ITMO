<template>
  <div id="app">
    ...
    <Middle :posts="posts" :users="users"
            :commentsMap="Object.values(comments).reduce((map, c) => {
          (map[c.postId] = map[c.postId] || []).push(c)
          return map
        }, {})"
            :postId="postId"/>
    <Footer :postsSize="Object.keys(posts).length" :usersSize="Object.keys(users).length"/>
  </div>
</template>
  ...
  beforeCreate() {
    ...
    this.$root.$on("onRegister", (login, name) => {
      ...
      if (!login || !/^[a-z]{3,16}$/.test(login)) {
        this.$root.$emit("onRegisterValidationError", "Login must be between 3 and 16 and contain only latin chars");
        ...
        this.$root.$emit("onRegisterValidationError", "Name must be between 1 and 32");
      } else {
