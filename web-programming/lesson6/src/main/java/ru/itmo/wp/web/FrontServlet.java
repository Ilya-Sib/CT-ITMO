...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route,
        ...
        Method beforeMethod = getMethod("before", pageClass);
        Method method = getMethod(route.getAction(), pageClass);
        Method afterMethod = getMethod("after", pageClass);
        ...
        try {
            callMethod(request, beforeMethod, page, view);
            callMethod(request, method, page, view);
            callMethod(request, afterMethod, page, view);
    ...
    private void callMethod(HttpServletRequest request, Method method, Object page, Map<String, Object> view)
            throws IllegalAccessException, InvocationTargetException {
        method.setAccessible(true);
        method.invoke(page, request, view);
    }

    private Method getMethod(String name, Class<?> pageClass) {
        Method resultMethod = null;
        for (Class<?> clazz = pageClass; resultMethod == null && clazz != null; clazz = clazz.getSuperclass()) {
            try {
                resultMethod = clazz.getDeclaredMethod(name, HttpServletRequest.class, Map.class);
