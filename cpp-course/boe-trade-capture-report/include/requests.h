...
constexpr size_t trade_capture_report_bitfield_num()
{
    return std::max({0
#define FIELD(_, n, __) , n
#include "trade_capture_report_opt_fields.inl"
    });
}

constexpr size_t trade_capture_report_opt_fields_size()
{
    return 0
#define FIELD(name, _, __) +name##_field_size
#include "trade_capture_report_opt_fields.inl"
...
constexpr size_t calculate_size(const RequestType type)
{
    ...
    case RequestType::TradeCaptureReport:
        return 56 + trade_capture_report_bitfield_num() + trade_capture_report_opt_fields_size();
    }
}
