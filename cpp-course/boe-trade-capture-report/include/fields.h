...
inline unsigned char * encode_trade_capture_report_price(unsigned char * start, const double value)
{
    const double order = 10000000;
    const double epsilon = 1e-8;
    // beware: no precision loss check
    return encode(start, static_cast<int64_t>(value * order + std::copysign(epsilon, value)));
}

inline unsigned char * encode_bitfields(unsigned char * start)
{
    *start++ = static_cast<unsigned char>(0x03);
    *start++ = static_cast<unsigned char>(0x01);
    *start++ = static_cast<unsigned char>(0x11);
    *start++ = static_cast<unsigned char>(0x20);
    return start;
}
